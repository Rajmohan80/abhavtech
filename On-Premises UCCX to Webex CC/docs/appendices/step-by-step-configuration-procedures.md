...
- 2.3 Configuring Responses
    ...
    !!! info "Deferred — See AI Implementation Chapter"
        Response template configuration (TTS settings, fallback messages, language variants) is covered in
        [10.3.4 Response Templates & TTS Settings](../chapter-10-ai-implementation/10-3-webex-ai-agent-configuration.md).
        Detailed click-by-click steps will be added once the Webex AI Agent UI is confirmed stable post-GA.
...
- 3.4 Configuring IAM Permissions
    ...
    !!! info "Deferred — Pending GCP Project Setup"
        IAM role assignments for the Dialogflow CX service account require the GCP project to be created first (3.1).
        Minimum required roles: `roles/dialogflow.admin` on the project, `roles/iam.serviceAccountTokenCreator` for the webhook service account.
        Full procedure will be drafted after GCP project provisioning is confirmed.
...
- 4.4 Creating Entities
    ...
    !!! info "Deferred — See NLU Design Chapter"
        Entity definitions (PRODUCT_TYPE, ACCOUNT_TYPE, LANGUAGE, CITY) are specified in
        [10.6 NLU & Intent Design](../chapter-10-ai-implementation/10-6-nlu-design-and-intent-development.md).
        Step-by-step Dialogflow CX console procedure will be added in Phase 2B implementation sprint.

- 4.5 Building Flows

    !!! info "Deferred — See Conversational Flows Chapter"
        Flow architecture and design patterns are in [10.7 Conversational Flows](../chapter-10-ai-implementation/10-7-conversational-flow-design.md).
        Console build procedure will be added after flow designs are signed off in UAT.

- 4.6 Creating Pages

    !!! info "Deferred — Linked to Flow Build"
...
- 4.8 Setting Up Environments
    ...
    !!! info "Deferred — Pre-Production Gate"
        Dialogflow CX environments (draft, staging, production) are configured after intent and flow development
        is complete. Required: draft environment promoted to staging after UAT sign-off, production promotion
        gated on Phase 2B go-live approval. Procedure will be documented at that stage.
...
- 5.2 Linking Dialogflow CX Agent
    ...
    !!! info "Deferred — Pending CCAI Connector Availability"
        The Webex Contact Center AI (CCAI) connector for Dialogflow CX requires the GCP project and Dialogflow
        agent to be fully configured first. Linking procedure: Control Hub > Contact Center > Integrations >
        Google CCAI > Add Integration. Full step-by-step will be added once the connector is activated in the tenant.

- 5.3 Configuring Voice Settings

    !!! info "Deferred — TTS Provider Selection Pending"
        Voice settings (TTS engine, voice profile, language, speaking rate) depend on the TTS provider decision
        (Google TTS vs Cisco TTS). Decision is tracked in [Deferred Items Register](deferred-items-register.md).
        Procedure will be drafted once provider is confirmed.
...
- 6.2 Configuring Node Parameters
    ...
    !!! info "Deferred — Flow Designer GA Required"
        Node parameter configuration varies by Flow Designer release. Parameters for Virtual Agent,
        HTTP Request, and Queue Contact nodes are documented in
        [6.5 IVR Flow Implementation](../chapter-06-implementation/6-5-ivr-flow-implementation.md).
        Appendix procedure will align once the production tenant is provisioned.

- [6.3 Handling Exit Events](#63-handling-exit-events)
- 6.4 Passing Context Variables

    !!! info "Deferred — See Webhook Integration Chapter"
        Context variable passing between Flow Designer and Dialogflow CX (via CCAI connector parameters)
        is specified in [10.8 Webhook & Fulfillment](../chapter-10-ai-implementation/10-8-webhook-and-fulfillment-integration.md).
        Implementation procedure will be added post-UAT.
